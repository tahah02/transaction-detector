...
def make_decision(txn, user_stats, model, features, autoencoder=None):
    ...
    if model is not None:
        # Prepare complete features for Isolation Forest (same as Autoencoder)
        ...
        # Complete feature set for Isolation Forest
        ml_features = { 
        ...
        vec = np.array([[ml_features.get(f, 0) for f in features]])
        
        # Handle NaN values (same as autoencoder)
