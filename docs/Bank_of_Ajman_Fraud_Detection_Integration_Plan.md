...
### Phase 1: Python API Deployment (Day 1)

#### 1.1 Server Preparation
- Install Python 3.9+ on Windows Server
...
# Service name: BankAjmanFraudDetection
# Port: 8001
# Auto-start: Yes
```
...
```sql
-- Create new tables in existing SQL Server database
...
-- Update existing transaction table
ALTER TABLE Transactions 
ADD FraudStatus NVARCHAR(50) DEFAULT 'NORMAL';
-- Possible values: NORMAL, PENDING_APPROVAL, APPROVED_BY_USER, REJECTED_BY_USER
...
### Phase 2: .NET Application Updates (Day 2)

#### 2.1 NuGet Package Installation
```xml
<!-- Add to existing .csproj file -->
...
public class FraudDetectionService : IFraudDetectionService
{
    ...
    public async Task<FraudDetectionResult> AnalyzeTransactionAsync(TransactionRequest request)
    {
        try
        {
            ...
            var response = await _httpClient.PostAsJsonAsync("analyze-transaction", fraudRequest);
            response.EnsureSuccessStatusCode();
    ...
    public async Task<bool> UpdateTransactionStatusAsync(string transactionId, string userDecision, string userId)
    {
        try
        {
            ...
            var response = await _httpClient.PostAsJsonAsync("update-transaction-status", updateRequest);
            return response.IsSuccessStatusCode;
        }
        catch (Exception ex)
        {
...
**TransactionController.cs - Modified Methods**
```csharp
[HttpPost("transfer")]
public async Task<IActionResult> ProcessTransfer(TransferRequest request)
{
...
**transaction.js - Enhanced Functions**
```javascript
// Enhanced transaction submission
async function submitTransfer(formData) {
    try {
...
// NEW: Fraud approval modal
function showFraudApprovalModal(data) {
    const modal = document.getElementById('fraudApprovalModal');
    
    // Populate modal content
    document.getElementById('riskScore').textContent = (data.RiskScore * 100).toFixed(1) + '%';
    document.getElementById('riskReasons').innerHTML = data.RiskReasons
        .map(reason => `<li>${reason}</li>`)
        .join('');
    ...
    try {
        ...
        if (result.Success) {
            ...
            if (decision === 'APPROVE') {
                ...
                showInfoMessage('Transaction has been cancelled');
            }
...
#### 3.2 HTML Modal Addition

**Add to existing transaction page**
```html
<!-- Fraud Approval Modal -->
<div id="fraudApprovalModal" class="modal" style="display: none;">
    <div class="modal-content">
        <div class="modal-header">
            <h3>⚠️ Security Check Required</h3>
        </div>
        <div class="modal-body">
            <p>Our security system has detected unusual patterns in this transaction.</p>
            
            <div class="risk-info">
                <div class="risk-score">
                    <label>Risk Score:</label>
                    <span id="riskScore" class="risk-value">0%</span>
                </div>
                
                <div class="risk-reasons">
                    <label>Reasons for Review:</label>
                    <ul id="riskReasons"></ul>
                </div>
            </div>
            
            <div class="approval-question">
                <p><strong>Do you want to proceed with this transaction?</strong></p>
            </div>
        </div>
        <div class="modal-footer">
            <button type="button" class="btn btn-success" onclick="handleApprovalDecision('APPROVE')">
                ✓ Yes, Proceed
            </button>
            <button type="button" class="btn btn-danger" onclick="handleApprovalDecision('REJECT')">
                ✗ Cancel Transaction
            </button>
        </div>
    </div>
...
### Phase 4: Python API Enhancements

#### 4.1 New Endpoints for .NET Integration

**api.py - Additional Endpoints**
```python
@app.post("/api/update-transaction-status")
def update_transaction_status(request: dict):
    """Update transaction status based on user decision"""
    try:
        ...
        if db_service.connect():
            db_service.update_fraud_result_status(
                transaction_id, user_decision, decided_by, decided_at
            )
        
        # Update local history
        stats_manager.update_transaction_status(transaction_id, user_decision)
        
        return {
            "success": True,
            "message": f"Transaction status updated to {user_decision}",
            "transaction_id": transaction_id
        }
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))

@app.get("/api/transaction-history/{customer_id}")
def get_customer_transaction_history(customer_id: str, limit: int = 50):
    """Get fraud detection history for a customer"""
    try:
        db_service = get_db_service()
        if db_service.connect():
            history = db_service.get_customer_fraud_history(customer_id, limit)
            return {
...
BACKUP DATABASE [BankAjmanDB] 
TO DISK = 'C:\Backups\BankAjmanDB_PreFraudIntegration.bak'
...
#### 2.2 Windows Service Installation
```bash
...
-- Verify stored procedures (if any)
```
...
# Verify fraud detection API URL
...
```csharp
// Enhanced logging in existing application
_logger.LogInformation("Fraud detection analysis started for transaction {TransactionId}", transactionId);
_logger.LogWarning("Suspicious transaction detected: {TransactionId}, Risk Score: {RiskScore}", 
    transactionId, riskScore);
_logger.LogError("Fraud detection API unavailable: {Error}", ex.Message);
```
...
BACKUP DATABASE [BankAjmanDB] 
TO DISK = 'C:\Backups\BankAjmanDB_' + FORMAT(GETDATE(), 'yyyyMMdd') + '.bak'
WITH COMPRESSION, CHECKSUM;
```
...
```bash
# Service won't start
# Check logs: C:\BankAjman\Logs\fraud_detection.log
...
// Check fraud detection API availability
...
ALTER TABLE Transactions DROP COLUMN FraudStatus;

-- Restore from backup (if needed)
RESTORE DATABASE [BankAjmanDB] 
FROM DISK = 'C:\Backups\BankAjmanDB_PreFraudIntegration.bak'
WITH REPLACE;
```
...
- ✅ Successful fraud detection rate > 95%

### Business Metrics
- ✅ Reduced fraudulent transactions
- ✅ Improved customer security
- ✅ Maintained transaction processing speed
...
This integration plan provides a comprehensive approach to deploying the fraud detection system into the existing Bank of Ajman solution. The phased approach ensures minimal disruption to current operations while adding robust security capabilities.

The solution maintains the existing user experience while adding an additional security layer that protects both the bank and its customers from fraudulent transactions.

**Estimated Timeline**: 2-3 days for complete deployment
**Risk Level**: Low (with proper testing and rollback procedures)
**Business Impact**: High (improved security and fraud prevention)
